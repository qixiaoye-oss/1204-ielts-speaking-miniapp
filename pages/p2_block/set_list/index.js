...
Page({
  ...
  toUnit(e) {
    ...
    wx.navigateTo({
      url: `/pages/p2_block/block_group/index?setId=${item.id}&dateLabel=${this.data.dateLabel.value}`,
    })
  },
  // ===========业务操作 End===========
  // ===========数据获取 Start===========
  ...
  listDateLabel() {
    api.request(this, '/story/citationsStatistics', { id: this.options.id }, false).then(res => {
      let dateLabelIndex = wx.getStorageSync('storyDateLabelIndex') || 0
      this.setData({
  ...
  changeDateLabel() {
    ...
    wx.showActionSheet({
      ...
      success(res) {
        ...
        wx.setStorageSync('storyDateLabelIndex', res.tapIndex)
        that.listData(true);
      }
    })
  }
